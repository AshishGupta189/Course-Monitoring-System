...
public class FacultyDaoImpl implements FacultyDao{
	...
	public String showFacultyAllocated(String s) {
		...
		try(Connection conn=dbutil.provideconnection()){
			...
			PreparedStatement ps=conn.prepareStatement("select f.facultyid,f.facultyname,b.batchid,b.cid from faculty f "
					+ "INNER JOIN batch b INNER JOIN Course c ON b.fid=f.facultyid AND c.courseid=b.cid AND "
					+ "c.courseName=?");
			ps.setString(1, s);
			ResultSet rs=ps.executeQuery();
			while(rs.next())
